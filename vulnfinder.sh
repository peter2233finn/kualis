...
regexFind "LOW: HTTP Security Headers are not present (on the nikto scan)." "header is not set|header is not present.|header is not defined." "nikto"
regexFind "LOW: Internal IP address exposed." "IP address found in the '.*' header" "nikto"
regexFind "LOW: Cross Origin Resource Sharing." "Access-Control-Allow-Origin:.*\*" "curl-to-root"
...
simpleFind "INFO: admin features found" "admin" "200"  "gobuster" 
...
simpleFind "INFO: SSL certificate validation dates" "Not valid"  "sslscan" 
